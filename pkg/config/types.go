...
// Cluster.ID uniquely identifies the cluster. Administrators may choose to use
// id2 or id3 to support different types of clients, who may refer to the
// cluster using different identifiers. Both client and server must agree on the
// same ID, and the client may only provide a single ID.
type Cluster struct {
	ID  string
	ID2 string
	ID3 string
...
type Config struct {
	...
	// Cluster is a unique-per-cluster identifier for your
